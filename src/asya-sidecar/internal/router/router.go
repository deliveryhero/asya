...
// Router handles meshage routing between queues and runtime client
type Router struct {
	cfg              *config.Config
...
// ensureAndUpdateStatus initializes or updates the status on a meshage before processing.
...
func (r *Router) ensureAndUpdateStatus(msg *meshages.Meshage) {
	now := time.Now().UTC().Format(time.RFC3339)
	maxAttempts := r.maxAttempts()

	if msg.Status == nil {
		msg.Status = &meshages.Status{
			Phase:       meshages.PhaseProcessing,
			Actor:       r.actorName,
			Attempt:     1,
	...
	msg.Status.Phase = meshages.PhaseProcessing
	msg.Status.Reason = ""
...
// effectiveTimeout computes the per-meshage timeout as the minimum of:
//   - ASYA_RESILIENCY_ACTOR_TIMEOUT (per-actor timeout, r.cfg.Timeout)
//   - remaining SLA (deadline_at - now, from meshage status)
func (r *Router) effectiveTimeout(msg *meshages.Meshage) time.Duration {
...
// shouldReportFinalToGateway returns false for meshages that must NOT trigger
// a final-status report to the gateway:
//  1. x-asya-fan-in header: fan-in partial batch (actor handles aggregation, not gateway)
//  2. parent_id set: fire-and-forget fan-out child (only root meshage is tracked by gateway)
//  3. non-terminal status.phase: human-in-the-loop or custom intermediate states
func (r *Router) shouldReportFinalToGateway(msg *meshages.Meshage) bool {
	if msg.Headers != nil {
	...
	if msg.Status != nil {
		...
		if phase != meshages.PhaseSucceeded && phase != meshages.PhaseFailed {
...
func (r *Router) processEndActorMeshage(ctx context.Context, msg meshages.Meshage, msgBody []byte, startTime time.Time) error {
	...
	// 1. Process the meshage via runtime
	// 2. Report final status to gateway
	// End actors run in meshage mode with validation disabled.
	...
	// Use original meshage payload if runtime returned empty/null
	if len(resultPayload) == 0 {
		resultPayload = msg.Payload
	}

	// Report final status to gateway if configured and meshage is terminal/not a fan-out child.
...
// parseAndValidateMessage parses and validates the meshage from message body
func (r *Router) parseAndValidateMessage(ctx context.Context, msgBody []byte, startTime time.Time) (*meshages.Meshage, error) {
	var msg meshages.Meshage
...
func (r *Router) handleErrorResponse(ctx context.Context, msg *meshages.Meshage, response runtime.RuntimeResponse, startTime time.Time) error {
	...
	if r.cfg.Resiliency == nil || r.cfg.Resiliency.Retry.MaxAttempts <= 1 {
		...
		return r.sendRetryFailure(ctx, msg, response, meshages.ReasonRuntimeError)
	}
	...
	if r.isNonRetryableError(response.Details.Type, response.Details.MRO) {
		...
		return r.sendRetryFailure(ctx, msg, response, meshages.ReasonNonRetryableFailure)
	}
	...
	if msg.Status.Attempt >= r.cfg.Resiliency.Retry.MaxAttempts {
		...
		return r.sendRetryFailure(ctx, msg, response, meshages.ReasonMaxRetriesExhausted)
	}
...
// retryMessage sends the meshage back to the actor's own queue with a delay.
// Updates the meshage status to reflect the retry state.
func (r *Router) retryMessage(ctx context.Context, msg *meshages.Meshage, details runtime.ErrorDetails, delay time.Duration) error {
	now := time.Now().UTC().Format(time.RFC3339)
	msg.Status.Phase = meshages.PhaseRetrying
	msg.Status.UpdatedAt = now
	msg.Status.Error = &meshages.StatusError{
		Type:      details.Type,
		MRO:       details.MRO,
...
// sendRetryFailure sends a failed meshage to the x-sump queue with proper
// retry status information (attempt count, reason, error details).
func (r *Router) sendRetryFailure(ctx context.Context, msg *meshages.Meshage, response runtime.RuntimeResponse, reason string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	...
	failedMsg := meshages.Meshage{
		ID:       msg.ID,
		ParentID: msg.ParentID,
		Route:    msg.Route,
		Payload:  payloadBytes,
		Status: &meshages.Status{
			Phase:       meshages.PhaseFailed,
			Reason:      reason,
			Actor:       r.actorName,
			Attempt:     attempt,
			MaxAttempts: r.maxAttempts(),
...
func (r *Router) routeToFlowErrorHandler(ctx context.Context, msg *meshages.Meshage, onError string, response runtime.RuntimeResponse, startTime time.Time) error {
...
func (r *Router) handleSuccessResponse(ctx context.Context, msg *meshages.Meshage, response runtime.RuntimeResponse, index int, runtimeDuration time.Duration) error {
	...
	// Use headers from runtime response; fall back to original meshage headers
	...
	if pauseRaw, ok := outHeaders["x-asya-pause"]; ok {
		...
		// Do NOT route to next actor — meshage is persisted by x-pause, gateway tracks state
		return nil
	}
...
// ProcessMessage handles a single meshage from the queue
func (r *Router) ProcessMessage(ctx context.Context, queueMsg transport.QueueMessage) error {
	...
	// SLA pre-check: reject expired meshages before processing
	...
	if timeout <= 0 || slaExpiring {
		...
		now := time.Now().UTC().Format(time.RFC3339)
		msg.Status.Phase = meshages.PhaseFailed
		msg.Status.Reason = meshages.ReasonTimeout
		msg.Status.UpdatedAt = now
...
func (r *Router) handleRuntimeCallError(ctx context.Context, msg *meshages.Meshage, err error, originalBody []byte, startTime time.Time) error {
...
func (r *Router) routeResponse(ctx context.Context, id string, parentID *string, route meshages.Route, payload json.RawMessage, inStatus *meshages.Status, headers map[string]interface{}) error {
	...
	var outStatus *meshages.Status
	now := time.Now().UTC().Format(time.RFC3339)
	if actorToSend != "" {
		outStatus = &meshages.Status{
			Phase:       meshages.PhasePending,
			Actor:       actorToSend,
			Attempt:     1,
			MaxAttempts: 1,
			UpdatedAt:   now,
		...
		outStatus = &meshages.Status{
			Phase:       meshages.PhaseSucceeded,
			Reason:      meshages.ReasonCompleted,
			Attempt:     1,
			MaxAttempts: 1,
			UpdatedAt:   now,
		}
...
// routes the meshage to x-sink. The caller must set msg.Status fields
// (Phase, Reason, etc.) before calling.
func (r *Router) handleSLAExpiry(ctx context.Context, msg meshages.Meshage, startTime time.Time) error {
	if r.metrics != nil {
...
// sendToSinkQueue sends the original meshage to the x-sink queue.
// If meshage.Status already has a terminal phase (succeeded/failed),
// it is preserved. Otherwise, PhaseSucceeded/ReasonCompleted is stamped.
func (r *Router) sendToSinkQueue(ctx context.Context, message meshages.Meshage) error {
	if message.Status == nil || (message.Status.Phase != meshages.PhaseSucceeded && message.Status.Phase != meshages.PhaseFailed) {
		now := time.Now().UTC().Format(time.RFC3339)
		createdAt := now
		if message.Status != nil {
			createdAt = message.Status.CreatedAt
		}
		message.Status = &meshages.Status{
			Phase:       meshages.PhaseSucceeded,
			Reason:      meshages.ReasonCompleted,
			Actor:       r.actorName,
			Attempt:     1,
...
// sendToSumpQueue sends an error meshage to the x-sump queue
func (r *Router) sendToSumpQueue(ctx context.Context, originalBody []byte, errorMsg string, errorDetails ...runtime.ErrorDetails) error {
	// Parse original meshage to extract id, parent_id, and route
	var originalMsg meshages.Meshage
	id := ""
	var parentID *string
...
// reportFinalStatusWithMessage reports final meshage status to gateway with full meshage context
// This is called by end actors (x-sink, x-sump) after processing
// It has access to both the meshage (with route) and the result payload
func (r *Router) reportFinalStatusWithMessage(ctx context.Context, msg *meshages.Meshage, resultPayload json.RawMessage, duration time.Duration) error {
	if r.progressReporter == nil {
		return nil
	}
...
func (r *Router) createFanoutMessage(ctx context.Context, id, parentID string, route meshages.Route) error {
