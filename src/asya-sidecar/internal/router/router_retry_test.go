...
func TestRouter_RetryMessage_SendsToOwnQueue(t *testing.T) {
	...
	msg := &meshages.Meshage{
		ID:      "msg-retry-1",
		Route:   meshages.Route{Prev: []string{}, Curr: "test-actor", Next: []string{"next-actor"}},
...
func TestRouter_ProcessMessage_RetryOnRetriableError(t *testing.T) {
	...
	inputMsg := meshages.Meshage{
		ID:      "test-retry-msg",
		Route:   meshages.Route{Prev: []string{}, Curr: "test-actor", Next: []string{"next"}},
...
func TestRouter_ProcessMessage_NonRetryableError(t *testing.T) {
	...
	var errorMsg meshages.Meshage
	if err := json.Unmarshal(mt.sentMessages[0].body, &errorMsg); err != nil {
		t.Fatalf("Failed to unmarshal: %v", err)
	}

	if errorMsg.Status.Phase != meshages.PhaseFailed {
		t.Errorf("Expected phase failed, got %s", errorMsg.Status.Phase)
	}
	if errorMsg.Status.Reason != meshages.ReasonNonRetryableFailure {
...
func TestRouter_ProcessMessage_MaxRetriesExhausted(t *testing.T) {
	...
	if errorMsg.Status.Reason != meshages.ReasonMaxRetriesExhausted {
...
func TestRouter_ProcessMessage_RetryPreservesPayloadAndRoute(t *testing.T) {
	...
	inputMsg := meshages.Meshage{
		ID:      "test-preserve",
		Route:   meshages.Route{Prev: []string{}, Curr: "test-actor", Next: []string{"next-actor", "final"}},
...
func TestRouter_SendRetryFailure_PreservesErrorDetailsInPayload(t *testing.T) {
	...
	err := r.sendRetryFailure(context.Background(), msg, response, meshages.ReasonMaxRetriesExhausted)
	if err != nil {
	...
	var failedMsg meshages.Meshage
	if err := json.Unmarshal(mt.sentMessages[0].body, &failedMsg); err != nil {
		t.Fatalf("Failed to unmarshal: %v", err)
	}

	// Verify status fields
	if failedMsg.Status.Phase != meshages.PhaseFailed {
		t.Errorf("Expected phase failed, got %s", failedMsg.Status.Phase)
	}
	if failedMsg.Status.Reason != meshages.ReasonMaxRetriesExhausted {
