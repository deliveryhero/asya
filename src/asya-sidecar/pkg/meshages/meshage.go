...
// Status tracks the lifecycle phase of a meshage as it moves through actors
type Status struct {
...
// Route represents the routing state of a meshage through the actor pipeline.
// prev: actors that have already processed this meshage (read-only to handlers).
// curr: the actor currently processing this meshage (read-only to handlers).
...
// Meshage represents the full meshage structure with routing metadata.
//
// Fanout ID Semantics:
...
// meshage for routing.
// The first fanout meshage retains the original ID to preserve SSE streaming compatibility.
// Subsequent fanout meshages receive UUID4 IDs to avoid collisions across concurrent fan-outs.
//
// Example fanout from meshage "abc-123" returning 3 items:
...
//
// All fanout children have ParentID set to the original meshage ID for traceability.
type Meshage struct {
	ID       string                 `json:"id"`
...
// Returns zero time and false if the meshage has no status, no deadline, or the value is malformed.
func (m *Meshage) ParseDeadline() (time.Time, bool) {
