...
def _parse_meshage_json(data: bytes) -> dict[str, Any]:
    """Parse received meshage from bytes to dict."""
    return json.loads(data.decode("utf-8"))


def _validate_meshage(
    e,  # type: dict
):
    # type: (...) -> dict
    if "payload" not in e:
        raise ValueError("Missing required field 'payload' in meshage")
    if "route" not in e:
        raise ValueError("Missing required field 'route' in meshage")
...
def _get_current_actor(meshage: dict) -> str:
    return meshage["route"]["curr"]
...
class _AbiContext:
    """ABI context for a single meshage invocation.

    Lifecycle: populate -> use -> snapshot -> discard.
    """

    def __init__(self, meshage):
        # type: (dict) -> None
        route = meshage["route"]
        self.data = {
            "id": meshage.get("id", ""),
            "parent_id": meshage.get("parent_id", ""),
            "route": {
                "prev": list(route["prev"]),
                "curr": route["curr"],
                "next": list(route["next"]),
            },
            "headers": dict(meshage.get("headers") or {}),
            "status": copy.deepcopy(meshage.get("status") or {}),
        }
...
def _collect_payload_frames(meshage, user_func):
    """Collect response frames using ABI dispatch for metadata."""
    ctx = _AbiContext(meshage)

    if inspect.isasyncgenfunction(user_func):
        return asyncio.run(_drive_async_generator(user_func(meshage["payload"]), ctx))

    if inspect.isgeneratorfunction(user_func):
        return _drive_generator(user_func(meshage["payload"]), ctx)

    # Function actor - no ABI access
    result = _call_handler(user_func, meshage["payload"])
    if result is None:
        return []
...
def _handle_invoke(data: bytes, user_func) -> tuple:
    ...
    Returns:
        ...
        (400, b'{"error": "..."}')   - meshage parsing/validation error
        (500, b'{"error": "..."}')   - handler raised an exception
    """
    try:
        meshage = _parse_meshage_json(data)
        if ASYA_ENABLE_VALIDATION:
            meshage = _validate_meshage(meshage)
    ...
    try:
        frames = _collect_payload_frames(meshage, user_func)
...
class _InvokeHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802
        ...
        try:
            meshage = _parse_meshage_json(body)
            if ASYA_ENABLE_VALIDATION:
                meshage = _validate_meshage(meshage)
            logger.debug(f"Received meshage: {len(body)} bytes")
        except (json.JSONDecodeError, UnicodeDecodeError, KeyError, ValueError) as exc:
        ...
        logger.info(f"[DIAG] Starting handler execution, meshage_id={meshage.get('id', 'unknown')}")

        if is_generator:
            self._stream_sse_response(meshage, user_func)
        else:
            try:
                frames = _collect_payload_frames(meshage, user_func)
            except Exception as exc:
                logger.exception("Fatal error on processing input meshage")
    ...
    def _stream_sse_response(self, meshage, user_func):
        ...
        try:
            if inspect.isasyncgenfunction(user_func):
                asyncio.run(_drive_async_generator(user_func(meshage["payload"]), ctx, on_fly=on_fly, on_emit=on_emit))
            else:
                _drive_generator(user_func(meshage["payload"]), ctx, on_fly=on_fly, on_emit=on_emit)
        except Exception as exc:
