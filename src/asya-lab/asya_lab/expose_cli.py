...
def _find_entrypoint(base_dir: Path) -> str:
    """Scan base/ YAML files for the actor with label asya.sh/flow-role: entry."""
    for yaml_file in sorted(base_dir.glob("*.yaml")):
        ...
        for doc in yaml.safe_load_all(text):
            ...
            if labels.get("asya.sh/flow-role") == "entry":
                actor_name = doc["metadata"]["name"]
                return actor_name

    click.echo("[-] No actor with label asya.sh/flow-role=entry found in base/", err=True)
    sys.exit(1)
