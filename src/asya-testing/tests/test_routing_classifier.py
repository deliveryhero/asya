...
async def test_start_router_chains_to_classifier_and_conditional(run_handler, monkeypatch, load_routers):
    ...
    assert "actor-router_routing_classifier_if_category_eq_billing" in actors
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("technical", "actor-router_routing_classifier_if_category_eq_technical"),  # passes to next router
    ],
)
async def test_billing_conditional_router(run_handler, monkeypatch, load_routers, category, expected_agent):
    """The billing router routes billing to billing_agent, others to next router."""
    routers = load_routers("routing_classifier")
    monkeypatch.setattr(routers, "resolve", lambda name: f"actor-{name}")

    payload = {"category": category, "message": "test"}
    result = await run_handler(routers.router_routing_classifier_if_category_eq_billing(payload))
...
async def test_account_general_conditional_router(run_handler, monkeypatch, load_routers):
    """The account router sends account to account_agent, else to general_agent."""
    routers = load_routers("routing_classifier")
    monkeypatch.setattr(routers, "resolve", lambda name: f"actor-{name}")

    # account path
    result = await run_handler(routers.router_routing_classifier_if_category_eq_account({"category": "account"}))
    actors = next(e[2] for e in result.abi if e[0] == "SET")
    assert "actor-account_agent" in actors

    # general (else) path
    result = await run_handler(routers.router_routing_classifier_if_category_eq_account({"category": "something-else"}))
    actors = next(e[2] for e in result.abi if e[0] == "SET")
