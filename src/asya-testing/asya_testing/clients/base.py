...
class TransportClient(ABC):
    ...
    @abstractmethod
    def publish(self, queue: str, meshage: dict) -> None:
        """
        Publish meshage to queue.

        Args:
            queue: Queue name
            meshage: Meshage dict (will be JSON-serialized)
        """
        pass
    ...
    def consume(self, queue: str, timeout: int = 10) -> dict | None:
        """
        Consume meshage from queue with timeout.
        ...
        Returns:
            Meshage dict or None if timeout
        """
        pass
...
class ActorTransportClient(TransportClient):
    ...
    def publish(self, queue: str, meshage: dict) -> None:
        """Publish meshage to actor queue."""
        queue_name = self._resolve_queue_name(queue)
        self._transport.publish(queue_name, meshage)

    def consume(self, queue: str, timeout: int = 10) -> dict | None:
        """Consume meshage from actor queue."""
        queue_name = self._resolve_queue_name(queue)
        return self._transport.consume(queue_name, timeout)

    def purge(self, queue: str) -> None:
        """Purge all meshages from actor queue."""
        queue_name = self._resolve_queue_name(queue)
