...
class RabbitMQClient(TransportClient):
    ...
    def publish(self, queue: str, meshage: dict, exchange: str = "asya") -> None:
        ...
        connection.close()
        logger.debug(f"Published to {queue}: {meshage.get('id', 'N/A')}")
    ...
    def consume(self, queue: str, timeout: int = 10) -> dict | None:
        ...
        logger.debug(f"Timeout waiting for meshage in {queue}")
        return None

    def purge(self, queue: str) -> None:
        """Purge all meshages from queue."""
