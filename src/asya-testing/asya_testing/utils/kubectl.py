...
def wait_for_asyncactor_ready(
    ...
    With function-auto-ready in the Composition pipeline, Crossplane sets
    Ready=True when all composed resources (Queue, ScaledObject, Deployment)
    are healthy.
    ...
    Returns:
        True if Ready condition is True, False if timeout
    """
    result = subprocess.run(
        [
            "kubectl",
    ...
    if result.returncode == 0:
        logger.info(f"AsyncActor {name} is Ready")
        return True
