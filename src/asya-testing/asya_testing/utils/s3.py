...
def find_meshage_in_s3(bucket_name: str, meshage_id: str, prefix: str = "") -> dict[str, Any] | None:
    """
    ...
    Args:
        bucket_name: Bucket name
        meshage_id: Message ID to search for
    ...
    for obj in objects:
        key = obj["Key"]
        if meshage_id in key:
            logger.info(f"Found message {meshage_id} at s3://{bucket_name}/{key}")
            return get_object_from_s3(bucket_name, key)

    logger.debug(f"Message {meshage_id} not found in bucket {bucket_name}")
    return None


def wait_for_meshage_in_s3(
    bucket_name: str,
    meshage_id: str,
    prefix: str = "",
    timeout: int = 5,
    ...
    attempt = 0
    logger.info(f"Polling S3 for message {meshage_id} in {bucket_name} (timeout={timeout}s)")

    while time.time() - start_time < timeout:
        attempt += 1
        message = find_meshage_in_s3(bucket_name, meshage_id, prefix)

        if message is not None:
            elapsed = time.time() - start_time
            logger.info(f"Found message {meshage_id} in S3 after {elapsed:.2f}s ({attempt} attempts)")
            return message

        time.sleep(poll_interval)  # Polling interval for S3 message check

    elapsed = time.time() - start_time
    logger.warning(f"Message {meshage_id} not found in S3 after {elapsed:.1f}s ({attempt} attempts)")
    return None
