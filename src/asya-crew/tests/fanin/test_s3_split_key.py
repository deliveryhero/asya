...
def test_full_cycle_two_slices(tmp_path):
    ...
    msg0 = make_meshage(origin_id, 0, 2, parent_payload)
    ...
    msg1 = make_meshage(origin_id, 1, 2, subagent_result)
...
def test_multi_slice_in_order_arrival(tmp_path):
    ...
    for i in range(slice_count - 1):
        ...
        msg = make_meshage(origin_id, i, slice_count, payload)
    ...
    msg_last = make_meshage(origin_id, slice_count - 1, slice_count, sub_results[-1])
...
def test_out_of_order_arrival(tmp_path):
    ...
    msg2 = make_meshage(origin_id, 2, slice_count, sub2)
    result = call_aggregator(msg2, base_dir)
    assert result is None

    # Slice 0 (parent) arrives second
    msg0 = make_meshage(origin_id, 0, slice_count, parent_payload)
    result = call_aggregator(msg0, base_dir)
    assert result is None

    # Slice 1 arrives last and triggers emission
    msg1 = make_meshage(origin_id, 1, slice_count, sub1)
    result = call_aggregator(msg1, base_dir)
...
def test_duplicate_slice_idempotent(tmp_path):
    ...
    msg1_first = make_meshage(origin_id, 1, slice_count, sub_result_v1)
    ...
    msg1_dup = make_meshage(origin_id, 1, slice_count, sub_result_v2)
...
def test_concurrent_completion_exactly_once(tmp_path):
    ...
    msg1 = make_meshage(origin_id, 1, slice_count, sub_result)
    result = call_aggregator(msg1, base_dir)
...
def test_transient_headers_not_in_payload(tmp_path):
    ...
    msg0 = make_meshage(origin_id, 0, slice_count, parent_payload, headers=all_headers)
    call_aggregator(msg0, base_dir)

    msg1 = make_meshage(origin_id, 1, slice_count, sub_result, headers=all_headers)
    result = call_aggregator(msg1, base_dir)
...
def test_aggregation_key_placement(tmp_path):
    ...
    msg0 = make_meshage(origin_id, 0, slice_count, parent_payload, aggregation_key=aggregation_key)
    call_aggregator(msg0, base_dir)

    msg1 = make_meshage(origin_id, 1, slice_count, sub1, aggregation_key=aggregation_key)
    call_aggregator(msg1, base_dir)

    msg2 = make_meshage(origin_id, 2, slice_count, sub2, aggregation_key=aggregation_key)
    result = call_aggregator(msg2, base_dir)
