...
- Primary table for job management
...
- Automatically cleaned up when job is deleted (CASCADE)

### Indexes

- `idx_jobs_status`: Fast filtering by job status
- `idx_jobs_created_at`: Sorting by creation time
- `idx_jobs_updated_at`: Finding recently updated jobs
- `idx_jobs_deadline`: Finding jobs approaching timeout
- `idx_job_updates_job_id`: Fast lookup of updates per job
- `idx_job_updates_timestamp`: Time-ordered update stream
