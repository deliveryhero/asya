...
func (r *Registry) createToolHandler(toolDef config.Tool) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		// Resolve route steps
		steps, err := toolDef.Route.GetSteps(r.config.Routes)
		if err != nil {
		...
		job := &types.Job{
			...
			Route: types.Route{
				...
				Metadata: map[string]interface{}{
					"job_id": jobID, // For terminal queue tracking
				},
			},
			Payload:    arguments,
		...
		if opts.Timeout > 0 {
			job.Deadline = time.Now().Add(opts.Timeout)
		}

		// Store job
		if err := r.jobStore.Create(job); err != nil {
		...
		go func() {
			...
			r.jobStore.Update(types.JobUpdate{
				JobID:     jobID,
			...
			if err := r.queueClient.SendMessage(context.Background(), job); err != nil {
			...
			log.Printf("Job %s sent to queue %s", jobID, steps[0])
		}()

		// Build response message
		message := fmt.Sprintf("Job created successfully with ID: %s\n\nUse the following endpoints:\n"+
			"- Status: GET /jobs/%s\n", jobID, jobID)

		// Add stream endpoint if progress is enabled
		if opts.Progress {
			message += fmt.Sprintf("- Real-time updates: GET /jobs/%s/stream (SSE)\n", jobID)
		}
