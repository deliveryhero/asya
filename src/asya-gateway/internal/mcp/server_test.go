...
func TestNewServer_WithoutConfig(t *testing.T) {
	...
	if server.registry != nil {
		t.Error("Registry should be nil when config is nil")
	}
	...
	cfg := &config.Config{
		Tools: []config.Tool{
			...
			},
			{
				...
				Route: config.RouteSpec{Steps: []string{"step4"}},
			},
		},
	}
				...
				Tools: []config.Tool{
					{
						...
						Parameters:  map[string]config.Parameter{},
						Route:       config.RouteSpec{Steps: []string{}},
					},
				},
			},
		},
		...
		},
		{
			...
			config: &config.Config{
				Tools: []config.Tool{
					...
					{
						Name:  "duplicate",
						Route: config.RouteSpec{Steps: []string{"step2"}},
					},
				},
			},
		},
