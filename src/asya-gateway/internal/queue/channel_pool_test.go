...
func TestChannelPool_Creation(t *testing.T) {
	...
	pool, err := NewChannelPool(url, "test-exchange", 5)
	if err != nil {
...
func TestChannelPool_GetReturn(t *testing.T) {
	url := "amqp://guest:guest@localhost:5672/"
	pool, err := NewChannelPool(url, "test-exchange", 3)
	if err != nil {
		t.Skipf("Skipping test - RabbitMQ not available: %v", err)
	}
	defer pool.Close()
...
func TestChannelPool_ConcurrentAccess(t *testing.T) {
	url := "amqp://guest:guest@localhost:5672/"
	poolSize := 10
	pool, err := NewChannelPool(url, "test-exchange", poolSize)
	if err != nil {
		t.Skipf("Skipping test - RabbitMQ not available: %v", err)
	}
	defer pool.Close()
...
func TestChannelPool_ContextCancellation(t *testing.T) {
	url := "amqp://guest:guest@localhost:5672/"
	pool, err := NewChannelPool(url, "test-exchange", 1)
	if err != nil {
		t.Skipf("Skipping test - RabbitMQ not available: %v", err)
	}
	defer pool.Close()
...
func TestChannelPool_CloseWhileInUse(t *testing.T) {
	url := "amqp://guest:guest@localhost:5672/"
	pool, err := NewChannelPool(url, "test-exchange", 2)
	if err != nil {
		t.Skipf("Skipping test - RabbitMQ not available: %v", err)
	}
...
func BenchmarkChannelPool_GetReturn(b *testing.B) {
	url := "amqp://guest:guest@localhost:5672/"
	pool, err := NewChannelPool(url, "bench-exchange", 10)
	if err != nil {
...
func BenchmarkChannelPool_PublishWithPool(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			err = ch.PublishWithContext(ctx, pool.exchange, "test-queue", false, false, amqp.Publishing{
