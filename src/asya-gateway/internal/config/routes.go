...
// RouteSpec can be either a string (template reference) or array of strings (explicit steps)
type RouteSpec struct {
	Steps    []string // Resolved route steps
...
// Allows route to be either: route: [step1, step2] OR route: template-name
...
// GetSteps resolves the route steps, using templates if specified
func (r *RouteSpec) GetSteps(templates map[string][]string) ([]string, error) {
	if len(r.Steps) > 0 {
		return r.Steps, nil
	}

	if r.Template != "" {
		steps, ok := templates[r.Template]
		if !ok {
	...
	return nil, fmt.Errorf("route has no steps or template")
}
...
func (t *Tool) Validate(templates map[string][]string) error {
	...
	// Validate route
	steps, err := t.Route.GetSteps(templates)
	if err != nil {
