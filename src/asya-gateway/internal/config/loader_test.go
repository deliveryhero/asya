		...
		Tools: []Tool{
			{
				...
				Route: RouteSpec{Steps: []string{"actor1"}},
			},
		},
		...
		Tools: []Tool{
			{
				...
				Route: RouteSpec{Steps: []string{"actor2"}},
			},
		},
		Routes: map[string][]string{
			"route2": {"step3", "step4"},
	...
	tests := []struct {
	...
	}{
		{
			...
			tool: Tool{
				...
				Route: RouteSpec{Steps: []string{"actor"}},
			},
			defaults: nil,
			want: ToolOptions{
				Progress: false,
...
`,
			check: func(r *RouteSpec) bool {
				return len(r.Steps) == 3 && r.Template == ""
			},
		},
		{
...
`,
			check: func(r *RouteSpec) bool {
				return r.Template == "my-template" && len(r.Steps) == 0
			},
		},
	}
