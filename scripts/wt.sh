#!/bin/sh
# Description: Manage worktrees: list, status, remove
subcmd="$1"; shift
...
if [ "$subcmd" = "list" ] || [ "$subcmd" = "ls" ]; then
  ...
  git aint get --output json --status all 2>/dev/null | python3 -c "
import json, sys
...
" | while read -r ref; do
  ...
  git aint set "$ref" --rm-tag "worktree:$wt_rel" --rm-tag "branch:$branch" || exit 1
