#!/bin/sh
# summarize.sh — generate structured project status overview
set -e
...
if output_fmt == "txt" and brief:
    ...
    for d in dir_names:
        ...
        for a in open_in_dir:
            ...
            lines.append(f"    {s:<8}{pri} [{ref}]  {title}{branch_part}{pr_part}")
        # recently closed
    ...
    for a in open_aints:
        ...
        branch = get_branch(a)
        branch_part = f"  branch:{branch}" if branch else ""
        lines.append(f"{s:<8}{pri} [{ref}] {d_part} {title}{branch_part}")
    lines.append("```")

elif output_fmt == "md" and not brief:
    lines.append("<!-- auto-generated by git aint summarize \u2014 do not edit -->")
    lines.append(f"# {header_line}")
    lines.append("")
    for d in dir_names:
        d_aints = dirs[d]
        ...
        open_in_dir.sort(key=lambda a: (a.get("priority", 4), a["status"] != "Active"))
        if open_in_dir:
            lines.append("| Status | Ref | Pri | Title | Branch | PR |")
            lines.append("|--------|-----|-----|-------|--------|----|")
            for a in open_in_dir:
                ...
                lines.append(f"| {sym} | [{ref}] | {pri} | {title} | {branch_cell} | {pr_cell} |")
            lines.append("")
        # recently closed
        closed = closed_items(d)
        if closed:
