...
**IMPORTANT**: Always use `make <target>` instead of direct commands. Add new Makefile targets for repeated tasks.
...
│   ├── asya-testing/        # Testing utilities and shared test code
│   └── asya-tools/          # Development and deployment tools
...
├── testing/                 # Test suites
│   ├── component/           # Component tests (single component + mocks)
...
### asya-sidecar (Go)
Envelope router injected into actor pods. Consumes from RabbitMQ, forwards to runtime via Unix socket, routes responses. Pluggable transport layer.

**Envelope Flow**: Queue → Sidecar → Runtime (Unix socket) → Sidecar → Next Queue

### asya-runtime (Python)
Lightweight socket server injected via ConfigMap. Loads user function, handles OOM recovery (RAM + CUDA), returns results or errors.
...
### asya-tools (Python)
Development and deployment utilities including scripts for manifest generation, debugging helpers, and cluster management tools.
...
2. **Component tests** (`make test-component`): Single component with lightweight mocks
   - **Location**: `testing/component/{component}/`
   - Uses Docker Compose for minimal infrastructure (RabbitMQ, mock services)
   - Tests single component behavior with real message queue
...
- Image loading to Kind: ~2-3 min
- Helmfile infrastructure (KEDA, RabbitMQ/SQS, MinIO/S3, Postgres, Operator, Gateway): ~5-10 min
- Helmfile application layer (actors): ~2-3 min
- Helm tests: ~1-2 min
- **Total: 15-25 minutes** before tests run

**Fast iteration workflow** (avoid full redeployment):
1. Patch code in `src/{component}/`
2. Rebuild only affected images: `make build-images`
...
**Test execution**:
- Run with fail-fast: `make trigger-tests PYTEST_OPTS="-vv -x" PROFILE="rabbitmq-minio" 2>&1 | tee /tmp/tests-foo.txt`
...
```bash
# Fix code
vim src/asya-sidecar/internal/transport/rabbitmq.go

# Rebuild and reload
make build-images
kind load docker-image asya-sidecar:latest --name asya-e2e-rabbitmq-minio

# Restart affected pods
kubectl rollout restart -n asya-e2e deployment/asya-gateway

# Run tests with fail-fast
make trigger-tests PYTEST_OPTS="-vv -x" PROFILE="rabbitmq-minio" 2>&1 | tee /tmp/tests.txt

# Check status every minute
watch -n 60 'tail -20 /tmp/tests.txt'
```
...
Sidecar increments `route.current` after processing.

**Automatic end routing** (NEVER configure explicitly):
- When `route.current` reaches end of `actors` array → sidecar routes to `happy-end` queue automatically
- When errors occur → sidecar routes to `error-end` queue automatically
...
**Before creating PR**: Run `make lint` and `make test` (must pass).

**Create PR**:
```bash
gh pr create --title "Title" --body "Summary, changes list, testing checklist"
```

**Addressing review comments**:
1. Read: `gh pr view <N> --json comments,reviews`
2. Fix issues, run `make lint && make test`
3. Reply to comments: `gh api -X POST repos/.../comments/<ID>/replies -f body="Fixed in <sha>"`
4. Commit: `git commit -m "Fix PR review comments\n- Fix 1\n- Fix 2"`
