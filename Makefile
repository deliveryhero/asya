...
setup: ## Set up development environment (install deps, pre-commit hooks)
	...
	@command -v go >/dev/null 2>&1 || (echo "[-] Go not found. Install Go 1.25+" && exit 1)
	...
	cd src/asya-scalers && go mod download && go mod tidy
...
test-unit: ## Run unit tests (go + python)
	...
	$(MAKE) -C src/asya-scalers test-unit
...
cov: ## Run all tests with coverage and display summary
	...
	$(MAKE) -C src/asya-scalers cov-unit
...
clean: clean-integration ## Clean build artifacts
	...
	PROFILE=sqs-s3-pvc $(MAKE) -C testing/e2e clean
	PROFILE=rabbitmq-minio-pg $(MAKE) -C testing/e2e clean
