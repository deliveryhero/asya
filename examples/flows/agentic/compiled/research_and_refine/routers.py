...
async def start_research_and_refine(payload: dict):
    ...
    _next.append(resolve("router_research_and_refine_while_loop"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_research_and_refine_if_iteration_ge_5(payload: dict):
...
async def router_research_and_refine_if_not_gaps_or_quality_score_ge_85(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if not p.get('gaps') or p.get('quality_score', 0) >= 85:
        ...
        _next.append(resolve("router_research_and_refine_if_iteration_ge_5"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_research_and_refine_seq_set_iteration(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['iteration'] += 1
    ...
    _next.append(resolve("router_research_and_refine_if_not_gaps_or_quality_score_ge_85"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_research_and_refine_while_loop(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    _next.append(resolve("router_research_and_refine_seq_set_iteration"))
    _next.append(resolve("router_research_and_refine_while_loop"))
