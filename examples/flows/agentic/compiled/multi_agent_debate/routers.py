...
async def start_multi_agent_debate(payload: dict):
    ...
    _next = []
    p['round'] = 0
    _next.append(resolve("router_multi_agent_debate_fanout_positions_2"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_agent_debate_fanout_positions(payload: dict):
    """Fan-out router: dispatches to sub-agents and aggregator (positions)"""
    p = payload

    origin_id = yield "GET", ".id"
    _next_tail = yield "GET", ".route.next"

    _agg = resolve("fanin_multi_agent_debate_positions")
...
async def router_multi_agent_debate_if_round_ge_3(payload: dict):
    ...
    if p['round'] >= 3:
        ...
        _next.append(resolve("router_multi_agent_debate_fanout_positions"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_agent_debate_if_converged(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p.get('converged'):
        ...
        _next.append(resolve("router_multi_agent_debate_if_round_ge_3"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_agent_debate_seq_set_round(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['round'] += 1
    _next.append(resolve("convergence_checker"))
    _next.append(resolve("router_multi_agent_debate_if_converged"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_agent_debate_while_loop(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    _next.append(resolve("router_multi_agent_debate_seq_set_round"))
    _next.append(resolve("router_multi_agent_debate_while_loop"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_agent_debate_fanout_positions_2(payload: dict):
    """Fan-out router: dispatches to sub-agents and aggregator (positions)"""
    p = payload
    ...
    _agg = resolve("fanin_multi_agent_debate_positions_2")
    ...
    # Index 0: parent payload forwarded to aggregator
    yield "SET", ".route.next", [_agg, resolve("router_multi_agent_debate_while_loop")] + _next_tail
    yield "SET", ".headers.x-asya-fan-in", {**_fan_in, "slice_index": 0}
    yield copy.deepcopy(p)
