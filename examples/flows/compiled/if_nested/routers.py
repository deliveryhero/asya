...
async def start_conditional_nested_flow(payload: dict):
    ...
    _next.append(resolve("router_conditional_nested_flow_if_a_eq_a"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_seq_set_x(payload: dict):
...
async def router_conditional_nested_flow_seq_set_x_2(payload: dict):
    ...
    _next.append(resolve("router_conditional_nested_flow_seq_set_x"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_seq_set_x_3(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 4
...
async def router_conditional_nested_flow_seq_set_x_4(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 3
    _next.append(resolve("handler_b"))
    _next.append(resolve("router_conditional_nested_flow_seq_set_x_3"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_seq_set_x_5(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 5
...
async def router_conditional_nested_flow_if_x_gt_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 2
    if p['x'] > 2:
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_4"))
    else:
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_5"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_seq_set_x_6(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 7
...
async def router_conditional_nested_flow_seq_set_x_7(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 6
    _next.append(resolve("handler_d"))
    _next.append(resolve("router_conditional_nested_flow_seq_set_x_6"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_if_x_gt_2_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['x'] > 2:
        _next.append(resolve("handler_b"))
        _next.append(resolve("router_conditional_nested_flow_if_x_gt_2"))
    else:
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_7"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_seq_set_x_8(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] += 1
    _next.append(resolve("handler_a"))
    _next.append(resolve("router_conditional_nested_flow_if_x_gt_2_2"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_conditional_nested_flow_if_a_eq_a(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['x'] = 1
    if p['a'] == 'A':
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_8"))
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_2"))
    else:
        _next.append(resolve("router_conditional_nested_flow_seq_set_x_2"))
