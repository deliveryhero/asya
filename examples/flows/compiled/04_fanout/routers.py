...
Regenerate by running: asya flow compile 04_fanout.py
"""
...
async def start_analysis_flow(payload: dict):
    ...
    _next = []
    _next.append(resolve("router_analysis_flow_fanout_analysis"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_analysis_flow_fanout_analysis(payload: dict):
    """Fan-out router: dispatches to sub-agents and aggregator (analysis)"""
    p = payload

    origin_id = yield "GET", ".id"
    _next_tail = yield "GET", ".route.next"

    _agg = resolve("fanin_analysis_flow_analysis")
