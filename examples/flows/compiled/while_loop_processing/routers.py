...
async def start_loop_flow(payload: dict):
    ...
    _next.append(resolve("router_loop_flow_seq_set_iteration_2"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_loop_flow_if_threshold_met(payload: dict):
...
async def router_loop_flow_seq_set_abort(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['abort'] = 2
    _next.append(resolve("router_loop_flow_while_iteration_lt_max_iterations"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_loop_flow_seq_set_abort_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['abort'] = 1
    _next.append(resolve("process_abort"))
    _next.append(resolve("router_loop_flow_seq_set_abort"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_loop_flow_if_skip_threshold_check(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p.get('skip_threshold_check'):
        _next.append(resolve("router_loop_flow_seq_set_abort_2"))
    else:
        _next.append(resolve("check_threshold"))
        _next.append(resolve("router_loop_flow_if_threshold_met"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_loop_flow_seq_set_iteration(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['iteration'] += 1
    _next.append(resolve("process_item"))
    _next.append(resolve("router_loop_flow_if_skip_threshold_check"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_loop_flow_while_iteration_lt_max_iterations(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['iteration'] < p['max_iterations']:
        _next.append(resolve("router_loop_flow_seq_set_iteration"))
        _next.append(resolve("router_loop_flow_while_iteration_lt_max_iterations"))
    else:
        yield "SET", ".route.next", [resolve("finalize_loop")]
        yield p
...
async def router_loop_flow_seq_set_iteration_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['iteration'] = 0
