...
async def start_multi_stage_flow(payload: dict):
    ...
    _next = []
    _next.append(resolve("router_multi_stage_flow_fanout_research"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_multi_stage_flow_fanout_reviews(payload: dict):
    """Fan-out router: dispatches to sub-agents and aggregator (reviews)"""
    p = payload

    origin_id = yield "GET", ".id"
    _next_tail = yield "GET", ".route.next"

    _agg = resolve("fanin_multi_stage_flow_reviews")
...
async def router_multi_stage_flow_fanout_research(payload: dict):
    """Fan-out router: dispatches to sub-agents and aggregator (research)"""
    p = payload

    origin_id = yield "GET", ".id"
    _next_tail = yield "GET", ".route.next"

    _agg = resolve("fanin_multi_stage_flow_research")
    ...
    # Index 0: parent payload forwarded to aggregator
    yield "SET", ".route.next", [_agg, resolve("router_multi_stage_flow_fanout_reviews")] + _next_tail
    yield "SET", ".headers.x-asya-fan-in", {**_fan_in, "slice_index": 0}
    yield copy.deepcopy(p)
