...
async def start_while_with_continue_flow(payload: dict):
    ...
    _next.append(resolve("router_while_with_continue_flow_seq_set_i"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_with_continue_flow_if_skip_iteration(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] += 1
    if p['skip_iteration']:
        _next.append(resolve("router_while_with_continue_flow_while_i_lt_max_iterations"))
    else:
...
async def router_while_with_continue_flow_while_i_lt_max_iterations(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['i'] < p['max_iterations']:
        _next.append(resolve("router_while_with_continue_flow_if_skip_iteration"))
        _next.append(resolve("router_while_with_continue_flow_while_i_lt_max_iterations"))
    else:
...
async def router_while_with_continue_flow_seq_set_i(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] = 0
