...
async def start_while_break_continue_flow(payload: dict):
    ...
    _next.append(resolve("router_while_break_continue_flow_seq_set_i_2"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_break_continue_flow_if_stop(payload: dict):
...
async def router_while_break_continue_flow_if_skip(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['skip']:
        _next.append(resolve("router_while_break_continue_flow_while_i_lt_max_iterations"))
    else:
        _next.append(resolve("handler_process"))
        _next.append(resolve("router_while_break_continue_flow_if_stop"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_break_continue_flow_seq_set_i(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] += 1
    _next.append(resolve("handler_check"))
    _next.append(resolve("router_while_break_continue_flow_if_skip"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_break_continue_flow_while_i_lt_max_iterations(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['i'] < p['max_iterations']:
        _next.append(resolve("router_while_break_continue_flow_seq_set_i"))
        _next.append(resolve("router_while_break_continue_flow_while_i_lt_max_iterations"))
    else:
        yield "SET", ".route.next", [resolve("handler_finalize")]
        yield p
...
async def router_while_break_continue_flow_seq_set_i_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] = 0
