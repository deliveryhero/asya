...
Source: try_except_nested.py

DO NOT EDIT THIS FILE MANUALLY
Regenerate by running: asya flow compile try_except_nested.py
"""
...
async def start_nested_error_handling(payload: dict):
    """Entrypoint for flow 'nested_error_handling'"""
    _next = []
    _next.append(resolve("fetch_data"))
    _next.append(resolve("parse_data"))
    _next.append(resolve("validate_data"))
    _next.append(resolve("transform_data"))
    _next.append(resolve("cleanup"))
    _next.append(resolve("store_results"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_nested_error_handling_except_connectionerror(payload: dict):
    """Router for error handling (except clause)"""
    p = payload
    p['status'] = 'connection_failed'
    yield "SET", ".route.next", [resolve('handle_connection_error'), resolve('cleanup'), resolve('store_results')]
    yield payload

async def router_nested_error_handling_except_valueerror(payload: dict):
    """Router for error handling (except clause)"""
    p = payload
    p['parse_status'] = 'invalid'
    yield "SET", ".route.next", [resolve('handle_parse_error'), resolve('transform_data')]
    yield payload
