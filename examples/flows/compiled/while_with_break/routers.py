...
async def start_while_with_break_flow(payload: dict):
    ...
    _next.append(resolve("router_while_with_break_flow_line_13_seq"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_with_break_flow_line_17_if(payload: dict):
...
async def router_while_with_break_flow_line_15_seq(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] += 1
    _next.append(resolve("handler_process"))
    _next.append(resolve("router_while_with_break_flow_line_17_if"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_with_break_flow_line_14_while_0(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['i'] < p['max_iterations']:
        _next.append(resolve("router_while_with_break_flow_line_15_seq"))
        _next.append(resolve("router_while_with_break_flow_line_14_while_0"))
    else:
        yield "SET", ".route.next", [resolve("handler_finalize")]
        yield p
        return
...
async def router_while_with_break_flow_line_13_seq(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] = 0
