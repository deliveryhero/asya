...
async def start_while_with_break_flow(payload: dict):
    ...
    _next.append(resolve("router_while_with_break_flow_seq_set_i_2"))
    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_with_break_flow_if_stop_condition(payload: dict):
...
async def router_while_with_break_flow_seq_set_i(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] += 1
    _next.append(resolve("handler_process"))
    _next.append(resolve("router_while_with_break_flow_if_stop_condition"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_while_with_break_flow_while_i_lt_max_iterations(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    if p['i'] < p['max_iterations']:
        _next.append(resolve("router_while_with_break_flow_seq_set_i"))
        _next.append(resolve("router_while_with_break_flow_while_i_lt_max_iterations"))
    else:
        yield "SET", ".route.next", [resolve("handler_finalize")]
        yield p
        return
...
async def router_while_with_break_flow_seq_set_i_2(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['i'] = 0
