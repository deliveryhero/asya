...
```yaml
# actor-image.lock (generated by asya actor lock / asya build)
version: 1
...
1. DS runs `asya promote my-actor`. The command:
   ...
   - **Compares**: if input_hash differs from lock, `asya promote` refuses:
     "Source changed since last build. Run `asya build` first."
...
# 3. If input_hash differs: ERROR "Source changed. Run asya build first."
...
Local working dir (ephemeral, uncommitted)
  -> asya build + asya k apply (staging, OCI-driven)
...
Build config (strategy + deps + code)
  ...
  |     asya build -> asya k apply --context stg
  |
