...
- **Meshage passing** each meshage contains "route" which allows actors send meshages each other
...
### asya-sidecar (Go)
Meshage router injected into actor pods. Consumes from RabbitMQ/SQS, forwards to runtime via Unix socket, routes responses. Pluggable transport layer.

**Meshage Flow**: Queue → Sidecar → Runtime (Unix socket) → Sidecar → Next Queue
...
**Meshage Metadata**: Generator handlers access meshage metadata (route, headers, status)
...
The Flow DSL compiler transforms Python-based workflow descriptions into router-based actor networks. Instead of manually managing meshage routes and creating router actors, you write familiar Python code with conditionals, and the compiler generates optimized router actors automatically.
...
{
  "id": "<meshage-id>",
  "parent_id": "<original-meshage-id>",  // optional, for fanout tracking
...
**Fields**:
- `id`: Unique meshage identifier
- `parent_id` (optional): Original meshage ID for fanout children (see docs/architecture/protocols/actor-actor.md)
...
**IMPORTANT**: All meshage queues are automatically managed by Crossplane providers (e.g., `provider-aws-sqs`).
