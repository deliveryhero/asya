...
- CRDs (install separately from `operator/config/crd/`)
- Actors (create AsyncActor resources after operator is running)
- KEDA (install separately if using autoscaling)

## CRD Management

The AsyncActor CRD is maintained in `../operator/config/crd/` and generated by Kubebuilder from Go API types.
...
kubectl delete -f ../operator/config/crd/
```
