...
* fix(sidecar): log runtime error content at ERROR level (#344) @atemate
* feat(compiler): per-scope semantics for context manager configs [ia37] (#342) @atemate
* feat(compiler): Phase 3 — flow composition, examples, validation (#340) @atemate
* feat(compiler): Phase 2 — 5-step pipeline with manifest generation and FlowInfo (#339) @atemate
* fix: Remove /mesh/expose — ConfigMap+hot-reload is the only tool registration mechanism [38we] (#335) @atemate
* feat(sidecar): policy-based error handling — policies+rules replaces retry+nonRetryableErrors (#334) @atemate
* fix(crew): x-sump must always be reached via x-sink (enforce two-layer termination) [nqf5] (#330) @atemate
* feat(sidecar): x-asya-first-attempt header for maxDuration tracking (#332) @atemate
* feat(compiler): union of PRs #278, #280, #307 — directives, decorators, rules engine (#329) @atemate
* fix(charts): all transports and IRSA disabled by default (#320) @atemate
* feat(dlq-worker): add GCS storage + Pub/Sub consumer for native GKE [y6xv] (#323) @atemate
* feat(xrd): remove transport from AsyncActor spec, use defaultCompositionRef (#321) @atemate
* fix(state-proxy): implement exclusive create (xb mode) in server and CAS connectors (#326) @atemate
* fix(e2e): fix three bugs introduced by @flow decorator commit (#315) (#322) @atemate
* fix(release): fix asya-playground packaged as 0.0.0 and trap exit code (#317) @atemate
...
* chore(deps): bump google.golang.org/grpc from 1.74.2 to 1.79.3 in /testing/integration/sidecar (#341) @[dependabot[bot]](https://github.com/apps/dependabot)
* fix(sidecar): log runtime error content at ERROR level (#344) @atemate
* docs: prettify repo for KubeCon (#343) @atemate
* feat(compiler): per-scope semantics for context manager configs [ia37] (#342) @atemate
* docs: restructure documentation against Diataxis framework [u1zh] (#338) @atemate
* feat(compiler): Phase 3 — flow composition, examples, validation (#340) @atemate
* feat(compiler): Phase 2 — 5-step pipeline with manifest generation and FlowInfo (#339) @atemate
* refactor(compiler): Phase 1 — replace ir/grouper/dotgen with analyzer/graphgen (#337) @atemate
* chore(deps): bump github.com/buger/jsonparser from 1.1.1 to 1.1.2 in /src/asya-gateway (#336) @[dependabot[bot]](https://github.com/apps/dependabot)
* fix: Remove /mesh/expose — ConfigMap+hot-reload is the only tool registration mechanism [38we] (#335) @atemate
* feat(sidecar): policy-based error handling — policies+rules replaces retry+nonRetryableErrors (#334) @atemate
* fix(crew): x-sump must always be reached via x-sink (enforce two-layer termination) [nqf5] (#330) @atemate
* chore: remove asya-actor helm chart (#333) @atemate
* feat(sidecar): x-asya-first-attempt header for maxDuration tracking (#332) @atemate
* docs: remove stale transport: field from AsyncActor spec examples (#331) @atemate
* feat(compiler): union of PRs #278, #280, #307 — directives, decorators, rules engine (#329) @atemate
* chore(deps): bump google.golang.org/grpc from 1.79.2 to 1.79.3 in /src/asya-crew/cmd/dlq-worker (#328) @[dependabot[bot]](https://github.com/apps/dependabot)
* chore(deps): bump google.golang.org/grpc from 1.74.2 to 1.79.3 in /src/asya-gateway (#327) @[dependabot[bot]](https://github.com/apps/dependabot)
* refactor(cli): remove asya flow \* subcommand, promote validate to top-level (#314) @atemate
* fix(charts): all transports and IRSA disabled by default (#320) @atemate
* refactor(gateway): unify terminology — task=A2A, tool=MCP, envelope=mesh (#325) @atemate
* feat(dlq-worker): add GCS storage + Pub/Sub consumer for native GKE [y6xv] (#323) @atemate
* refactor(runtime): cleanup asya\_runtime.py (#324) @atemate
* feat(xrd): remove transport from AsyncActor spec, use defaultCompositionRef (#321) @atemate
* fix(state-proxy): implement exclusive create (xb mode) in server and CAS connectors (#326) @atemate
* fix(e2e): fix three bugs introduced by @flow decorator commit (#315) (#322) @atemate
* docs(gcp-gke): complete GKE + GCP Pub/Sub install guide for KubeCon demo (#319) @atemate
* fix(release): fix asya-playground packaged as 0.0.0 and trap exit code (#317) @atemate
* feat(compiler): require @flow decorator on flow entry points (#315) @atemate
* feat(branding): add project logos to README and website (#313) @atemate
...
- `ghcr.io/deliveryhero/asya-crew:0.5.6`
- `ghcr.io/deliveryhero/asya-gateway:0.5.6`
- `ghcr.io/deliveryhero/asya-sidecar:0.5.6`
- `ghcr.io/deliveryhero/asya-testing:0.5.6`
...
[Unreleased]: https://github.com/deliveryhero/asya/compare/v2.0.0...HEAD
[2.0.0]: https://github.com/deliveryhero/asya/releases/tag/v2.0.0
