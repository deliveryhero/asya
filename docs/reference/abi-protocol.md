...
- **Read meshage metadata** (where did this meshage come from?)
...
Read a field from the meshage metadata. Returns a **deep copy** — mutating
the returned value does not affect the meshage.
...
Remove a field from the meshage metadata.
...
separate downstream meshage (fan-out):
...
Paths use **jq-like dot notation** rooted at the meshage envelope.

### Dot access

```
.route.next       → meshage["route"]["next"]
.headers.trace_id → meshage["headers"]["trace_id"]
.route            → meshage["route"]  (entire subtree)
```

The leading `.` is required and refers to the meshage root.
...
The meshage **payload** is not accessible via the ABI — it's the function
argument itself. The ABI operates on meshage metadata only.
...
This mirrors the actual Asya execution: each `await` represents a meshage
...
- [Architecture: Actor Meshage Protocol](../architecture/protocols/actor-actor.md) —
  meshage envelope format and routing semantics
