...
Source: ../src/demo_flows/text_improver.py

DO NOT EDIT THIS FILE MANUALLY
Regenerate by running: asya flow compile ../src/demo_flows/text_improver.py
"""
...
async def start_text_improver(payload: dict):
    ...
    p = payload
    p['iteration'] = 0
    _next.append(resolve("router_text_improver_line_20_loop_back_0"))
    yield "SET", ".route.next[:0]", _next
    yield p

async def router_text_improver_line_29_if(payload: dict):
...
async def router_text_improver_line_26_if(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
...
async def router_text_improver_line_21_seq(payload: dict):
    """Router for control flow and payload mutations"""
    p = payload
    _next = []
    p['iteration'] += 1
    ...
    _next.append(resolve("router_text_improver_line_26_if"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def router_text_improver_line_20_loop_back_0(payload: dict):
    """Loop-back router: re-inserts loop actors into route (guarded)"""
    p = payload
    _next = []
    _self = resolve("router_text_improver_line_20_loop_back_0")
    _prev = yield "GET", ".route.prev"
    if _prev.count(_self) >= _ASYA_MAX_LOOP_ITERATIONS:
        raise RuntimeError(f"Max loop iterations ({_ASYA_MAX_LOOP_ITERATIONS}) exceeded for while-loop at line 20")

    _next.append(resolve("router_text_improver_line_21_seq"))
    _next.append(resolve("router_text_improver_line_20_loop_back_0"))

    yield "SET", ".route.next[:0]", _next
    yield payload

async def end_text_improver(payload: dict):
    """Exitpoint for flow 'text_improver'"""
