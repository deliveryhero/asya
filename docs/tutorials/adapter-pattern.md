...
```python
# myapp/handlers.py
from myapp.llm import call_llm, call_llm_stream  # your domain functions
...
### The `run_handler` fixture

`asya-testing` provides a `run_handler` pytest fixture that drives any handler
(function or generator) and captures **all** emitted events — downstream
frames, FLY streaming events, and ABI commands — without filtering any of them.

```python
# conftest.py (or install asya-testing and import from asya_testing.fixtures)
from asya_testing.fixtures import run_handler  # re-export for pytest discovery
```

The fixture returns a `HandlerResult` with three fields:

| Field | Contents |
|-------|----------|
| `result.frames` | Downstream payloads — dicts emitted to the next actor |
| `result.fly` | Upstream SSE events — data from every `yield "FLY", {...}` |
| `result.abi` | Other ABI commands — every `("SET", ...)`, `("GET", ...)`, `("DEL", ...)` tuple |
| `result.payload` | Shortcut: `frames[0]`, raises if not exactly one frame |
...
async def test_order_approved(run_handler):
    result = await run_handler(process_order_actor({"order_id": "ord-001", "amount": 500.0}))

    assert result.payload["approved"] is True
    assert result.payload["fee"] == pytest.approx(10.0)
    # Original fields are preserved
    assert result.payload["order_id"] == "ord-001"

async def test_order_rejected_high_amount(run_handler):
    result = await run_handler(process_order_actor({"order_id": "ord-002", "amount": 15_000.0}))

    assert result.payload["approved"] is False

async def test_invalid_input_raises(run_handler):
    with pytest.raises(Exception):
        await run_handler(process_order_actor({"amount": -1.0}))  # missing order_id
```

### Testing routing decisions

To verify that your generator adapter sets routing correctly, check `result.abi`:
...
async def test_urgent_routing(run_handler):
    state = {"text": "CRITICAL: system down"}

    with patch("myapp.handlers.classify", return_value="urgent"):
        result = await run_handler(classifier_actor(state))

    assert ("SET", ".route.next", ["escalation-handler", "notifier"]) in result.abi
    assert result.payload["label"] == "urgent"

async def test_standard_routing(run_handler):
    state = {"text": "Monthly report attached"}

    with patch("myapp.handlers.classify", return_value="normal"):
        result = await run_handler(classifier_actor(state))

    assert ("SET", ".route.next", ["standard-handler"]) in result.abi
```

### Testing FLY streaming

To verify your generator streams the right tokens, check `result.fly`:
...
async def test_llm_streams_tokens(run_handler):
    state = {"query": "What is Asya?", "events": []}

    with patch("myapp.handlers.call_llm_stream") as mock_stream, \
         patch("myapp.handlers.call_llm", new_callable=AsyncMock) as mock_llm:
        mock_stream.return_value = aiter(["Asya ", "is ", "great."])
        mock_llm.return_value.content = "Asya is great."
        mock_llm.return_value.tool_calls = []

        result = await run_handler(llm_actor(state))

    # Verify streaming events were emitted
    assert len(result.fly) == 3
    assert result.fly[0] == {"type": "text_delta", "token": "Asya "}

    # Verify the response was appended to the conversation history
    assert len(result.payload["events"]) == 1
    assert result.payload["events"][0]["content"] == "Asya is great."
```

### Testing a multi-actor flow

Chain adapter calls to test end-to-end behavior without any infrastructure.
Each `await run_handler(...)` simulates one message hop through the actor mesh:

```python
async def test_full_pipeline(run_handler):
    state = {"order_id": "ord-100", "amount": 250.0, "text": "process order"}

    # Hop 1: order processing
    result = await run_handler(process_order_actor(state))
    state = result.payload

    # Hop 2: classification
    with patch("myapp.handlers.classify", return_value="normal"):
        result = await run_handler(classifier_actor(state))
    state = result.payload
...
In production, each hop crosses a queue boundary between pods. In tests, they
run sequentially in one process — the behavior is identical because adapters
are pure Python.
...
- [ABI Protocol Reference](../reference/abi-protocol.md) — full verb reference,
  path syntax, and the `actor()` helper for running flows locally outside of tests
