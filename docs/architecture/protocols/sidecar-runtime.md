...
Before calling the runtime, the sidecar checks `status.deadline_at` on the incoming meshage. If the current time is past the deadline, the meshage is routed directly to `x-sink` with `phase=failed`, `reason=Timeout` — the runtime is never called.

The gateway stamps `status.deadline_at` based on the tool's `timeout_seconds` configuration. This absolute deadline is never mutated as the meshage travels through actors.

### Effective Timeout (Per-Call)

For meshages that pass the SLA pre-check, the sidecar computes an effective timeout:
...
1. Sidecar sends the meshage to `x-sump` with a timeout error
...
3. **Return `None` to abort** — handlers returning `None` produce a `204` response, which routes the meshage to `x-sink` without an error. Use this for intentional pipeline exits, not errors.
...
5. **Test failure modes in staging** before production: inject bad payloads, simulate timeouts, and verify meshages land in `x-sump` rather than disappearing silently.
