# RFC: AI Agent Framework Handler Syntax Comparisons

> **Purpose**: Survey how major AI agent/tool frameworks define handlers, tools, agents, and data flow.
> Inform Asya's handler syntax design decisions and potential wrapper/translator strategies.
>
> **Date**: 2026-02-09
...
4. **Core capabilities** -- streaming, memory, persistence, orchestration
...
**Relevance to Asya**: ADK's `output_key` pattern is essentially the same enrichment pattern Asya uses -- each actor's output goes to a named key in shared state. The difference is ADK configures the key on the agent definition, not in a flow.
...
**Relevance to Asya**: Mastra's Zod schema approach is the TypeScript equivalent of Pydantic. If Asya wants to support JS actors in the future, Zod schemas would be the natural input/output definition mechanism.
...
**Relevance to Asya**: LangGraph's state reducer pattern (`Annotated[list, add]`) is conceptually similar to Asya's payload enrichment -- state accumulates through the pipeline. The difference is LangGraph uses in-process shared memory, while Asya uses serialized envelope payloads.
...
This means **Asya's current `payload: dict` handler contract is the outlier**. Every other framework lets you write:
...
def process(payload: dict) -> dict:
    return {"weather": get_weather(payload["city"])}
```
...
| LlamaIndex | `ctx: Context` | Workflow context |
| Agno | `run_context: RunContext` | Session state access |
| AutoGen | `cancellation_token: CancellationToken` | Cancellation control |
| LangGraph | `state: Annotated[dict, InjectedState]` | Graph state |

The parameter is **auto-detected by name or type annotation** and excluded from the tool's public schema. This is a clean pattern that Asya could adopt for envelope/state access.
...
This validates Asya's enrichment pattern as the right foundation. The question is how to make it implicit with typed handlers.
...
## Implications for Asya

### Current Asya Handler Contract
...
def process(envelope: dict) -> dict:
    return {"payload": ..., "route": ..., "headers": ...}
```

### What Other Frameworks Do Differently

1. **Typed parameters instead of dict**: `def get_weather(city: str) -> str` vs `def process(payload: dict) -> dict`
2. **Docstring as description**: All frameworks use docstrings for tool descriptions
3. **No framework imports**: Most frameworks keep tools as plain functions (Asya already does this)
4. **State injection via magic parameters**: Not via the payload dict itself
5. **Schema auto-generation**: From type hints, not manual definition

### Potential Wrapper/Translator Strategies

Given the universal `function + type hints + docstring` pattern, Asya could provide:

1. **Import adapters**: `asya.from_langchain(tool)`, `asya.from_crewai(tool)`, etc.
2. **Signature-based auto-wrapping**: Detect typed functions and auto-generate payload extraction/injection
3. **MCP compatibility**: Several frameworks already support MCP tools -- Asya's MCP gateway could serve tools to these frameworks
4. **Decorator-free support**: Like Google ADK and LlamaIndex, allow plain functions as handlers

### ADK's `output_key` as a Model for Asya

Google ADK's `output_key` pattern maps directly to Asya's enrichment needs:
- Each actor's output goes to a named key in the payload
- The key is configured at deployment time (in the CRD), not in the handler code
- Downstream actors read from the key by parameter name

This could work for Asya:
```yaml
# AsyncActor CRD
spec:
  handler: handlers.detect_timezone
  handlerMode: typed          # New mode
  outputKey: timezone         # Where to store the result in payload
```

The sidecar would:
1. Inspect handler signature at startup
2. Extract matching args from payload
3. Call handler with typed args
4. Store result under `outputKey` in payload

---

## Summary: Framework Philosophy Spectrum
...
**Asya's position**: Asya is unique as a **distributed choreography framework** -- none of these frameworks decompose into separate processes/containers communicating via message queues. They all run in-process. Asya's typed handler support should make it easy to take functions written for any of these frameworks and deploy them as distributed actors, while the flow compiler handles the data flow plumbing.
