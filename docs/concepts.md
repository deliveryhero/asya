...
- Meshage routing between queues and runtime
- Transport management (RabbitMQ, SQS)
- Observability (metrics, logs)
- Reliability (retries, error handling)

**How it works**: Injected as a container into actor pods. Consumes meshages from queues, validates meshage structure, forwards to runtime via Unix socket, routes responses to next queue.
...
**How it works**: Receives meshages from sidecar via Unix socket, loads user handler (function or class), executes it, returns results back to sidecar.
...
**Stateful routing**: `route.current` increments after each actor processes the meshage. Note once again, this is a unique feature of 🎭: pipelines are stateless, but meshages are stateful (they represent different pipeline executions).
