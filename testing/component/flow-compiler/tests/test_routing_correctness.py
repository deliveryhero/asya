...
def _compile_and_exec(source, env_vars=None):
    ...
    Generated router functions are auto-registered as ASYA_HANDLER_*
    env vars so resolve() can find them during execution.
    """
    if env_vars:
    ...
    # Auto-register generated routers so resolve() can find them
    import re

    for match in re.finditer(r"^async def (\w+)\(", code, re.MULTILINE):
        func_name = match.group(1)
        if func_name == "resolve":
            continue
        if any(func_name.startswith(p) for p in _ROUTER_PREFIXES):
            env_key = f"ASYA_HANDLER_{func_name.upper()}"
            os.environ[env_key] = f"routers.{func_name}"
...
class TestMutationRouting:
    ...
    def test_multiple_mutations(self):
        ...
        namespace = _compile_and_exec(source)
        # Mutations live in the sequence router, not start_flow
        seq_name = [n for n in namespace if n.startswith("router_") and "_seq" in n][0]
        seq_func = namespace[seq_name]
...
class TestRouteInsertion:
    ...
    def test_router_prepends_to_next(self):
        ...
        msg_ctx = _make_msg_ctx(route_next=["existing_actor"])
        _drive_abi(start_func({}), msg_ctx)
        next_actors = msg_ctx["route"]["next"]
        # Start router prepends its routing, preserving existing route
        assert next_actors[-1] == "existing_actor"
