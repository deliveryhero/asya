...
# Check if a port is already reachable (e.g. via NodePort)
port_reachable() {
  local port=$1
  curl -sf -m 2 "http://127.0.0.1:${port}/" > /dev/null 2>&1 ||
...
start_port_forwards() {
  ...
  if port_reachable 8080; then
    echo "[+] Gateway already reachable (localhost:8080) — NodePort or existing forward"
  else
  ...
  if kubectl get svc -n "$SYSTEM_NAMESPACE" localstack-s3 > /dev/null 2>&1; then
    if port_reachable "$S3_LOCAL_PORT"; then
      echo "[+] S3 already reachable (localhost:$S3_LOCAL_PORT) — NodePort or existing forward"
    else
  ...
  if kubectl get svc -n "$SYSTEM_NAMESPACE" localstack-sqs > /dev/null 2>&1; then
    if port_reachable 4566; then
      echo "[+] SQS already reachable (localhost:4566) — NodePort or existing forward"
    else
  ...
  if kubectl get svc -n "$NAMESPACE" asya-rabbitmq > /dev/null 2>&1; then
    if port_reachable 5672; then
      echo "[+] RabbitMQ already reachable (localhost:5672) — NodePort or existing forward"
    else
  ...
  if kubectl get svc -n "$NAMESPACE" minio > /dev/null 2>&1; then
    if port_reachable 9000; then
      echo "[+] MinIO already reachable (localhost:9000) — NodePort or existing forward"
    else
