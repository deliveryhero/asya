...
class RabbitMQTestHelper:
    ...
    def publish_meshage(
        self, queue: str, meshage: dict, exchange: str = "asya"
    ) -> None:
    ...
    def assert_meshage_in_queue(
        ...
        """Assert that a meshage appears in the specified queue."""
        message = self.get_meshage(queue, timeout)
...
class SQSTestHelper:
    ...
    def publish_meshage(
        self, queue: str, meshage: dict, exchange: str = ""
    ) -> None:
        """Publish meshage to SQS queue."""
        logger.debug(f"Publishing to queue='{queue}'")
        logger.debug(f"Message body: {json.dumps(meshage)[:200]}...")
