...
def test_retry_max_attempts_exhausted(transport_helper):
    ...
    result = transport_helper.get_meshage(SUMP_QUEUE, timeout=30)
...
def test_retry_non_retryable_error(transport_helper):
    ...
    result = transport_helper.get_meshage(SUMP_QUEUE, timeout=10)
    logger.info(f"Result from x-sump: {json.dumps(result, indent=2) if result else 'None'}")
...
def test_retry_success_after_one_failure(transport_helper):
    ...
    transport_helper.publish_meshage(RETRY_SUCCEED_QUEUE, meshage)

    result = transport_helper.get_meshage(SINK_QUEUE, timeout=20)
...
def test_retry_attempt_resets_on_actor_transition(transport_helper):
    ...
    result = transport_helper.get_meshage(SINK_QUEUE, timeout=30)
