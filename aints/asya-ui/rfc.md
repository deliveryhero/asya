# RFC: @asya/ui — React Component Library & UI Architecture
...
**Source location**: `src/asya-lab/ui/` — colocated with the Python package but
separate build toolchain:

```
src/asya-lab/
├── asya_lab/              ← Python package (magics, compile, serve)
│   ├── serve/             ← asya serve (FastAPI)
│   ├── jupyter/           ← anywidget wrappers
│   └── static/            ← built JS copied here for Jupyter wheel
├── ui/                    ← @asya/ui (React, npm package)
│   ├── package.json       ← name: "@asya/ui"
│   ├── src/
│   │   ├── components/
│   │   ├── providers/
│   │   └── tokens/
│   ├── dist/              ← ESM + CJS bundles (build output)
│   └── tsconfig.json
├── pyproject.toml
└── Makefile               ← `make build` runs uv build + npm run build
```

Why colocated: `asya_lab/serve/` references `../ui/dist/` for the SPA,
the Jupyter build copies `ui/dist/flow_widget.js` into `asya_lab/static/`,
and one Makefile target builds both. The VSCode extension references
`@asya/ui` as `"file:../asya-lab/ui"` during development or from npm in CI.
...
**Node classification — two axes**:

1. **type** — rendering style:
   - `router` — auto-generated by compiler (wheat fill)
   - `actor` — user-defined handler (blue fill)

2. **role** — semantic function:
   - `conditional` — if/elif/else branch point
   - `mutation` — batched payload mutations
   - `loop` — while/while-True condition
   - `fanout` — parallel dispatch
   - `fanin` — convergence/aggregation
   - `processor` — user handler logic
   - `raise_exit` — re-raise terminal (only in try-except flows)

3. **entrypoint** / **exitpoint** — boolean flags, orthogonal to type/role.
   Mark the graph's start and end nodes (important for visual emphasis).
   A node can be both `type: router` and `entrypoint: true` (e.g., first
   mutation router), or `type: actor` and `exitpoint: true` (e.g., last
   handler in a flow with no trailing router).

**Edge types** — 5 kinds, no more:

| Type | Label | Meaning |
|---|---|---|
| `sequential` | — | Unconditional flow (A → B) |
| `true` | "TRUE" | Condition true branch |
| `false` | "FALSE" | Condition false branch |
| `except` | "except Type" | Error path from try block |
| `fanout` | "slice N" or "slice 1..N" | Fan-out dispatch |

All other transitions (loop-back to while condition, fan-in convergence,
raise terminal, finally continuation) use `sequential` — they're visually
obvious from the graph topology without special edge styling.

**Groups** — visual clusters for try/finally blocks:

```json
"groups": [
  {"id": "try-0", "type": "try", "nodes": ["parse-input", "transform-data"]},
  {"id": "finally-0", "type": "finally", "nodes": ["cleanup"]}
]
```

**Internal router nodes omitted**: `try_enter`, `try_exit`, `except_dispatch`,
`reraise` are code-generation details. React Flow shows the logical flow —
actors inside a `try` group with error edges drawn from the group boundary.
  ...
  "nodes": [
    {
      ...
      "entrypoint": true,
      "label": "initialize",
      "handler": "compiled_routers.router_init",
    ...
    },
    {
      ...
      "type": "actor",
      "role": "processor",
      "label": "express_handler",
      "handler": "handlers.express_handler"
    },
    {
      "id": "standard-handler",
      "type": "actor",
      ...
      "handler": "handlers.standard_handler"
    },
    {
      ...
      "handler": "handlers.payment_processor"
    }
  ],
  "edges": [
    {"source": "router-init", "target": "validate-order", "type": "sequential"},
    {"source": "validate-order", "target": "cond-order-type", "type": "sequential"},
    {"source": "cond-order-type", "target": "express-handler", "type": "true"},
    {"source": "cond-order-type", "target": "standard-handler", "type": "false"},
    {"source": "express-handler", "target": "payment-processor", "type": "sequential"},
    {"source": "standard-handler", "target": "payment-processor", "type": "sequential"}
  ],
  "groups": []
}
```
...
**Node fill** reflects type (two groups, matching DOT):
| Type | Fill | DOT equivalent |
|---|---|---|
| `router` (auto-generated) | Light wheat (#fefce8) | wheat |
| `actor` (user-defined) | Light blue (#eff6ff) | lightblue |

Entrypoint/exitpoint nodes get a **thicker border** (3px vs 1px) instead of
a different fill — keeps the two-group coloring clean while visually marking
graph start/end.
