...
Output path is configurable via `asya.template.output` in config.yaml (default:
...
2. **Interpolation resolves AFTER merge**: Top-level config references
   resolve lazily after the full effective config is assembled. This is
   why a child config can reference `${project_root}` even though it's
   only defined in the root config. OmegaConf's lazy resolution makes
   this work.
3. **Colon resolvers (`${arg:*}`, `${actor:*}`, `${env:*}`) resolve at
   command time**: After merge and after top-level resolution. Missing
   `arg` values are a hard error (no defaults). `${actor:*}` values are
   populated by the compiler — they are unavailable outside compile context.
4. **Root config MUST define top-level constants**: Constants like
   `project_root`, `image_registry` are auto-generated by `asya init` in
...
asya:
  build:
    ...
    - module: shared_utils
      path: "${project_root}/libs/shared_utils"
      image: "${image_registry}/shared:${arg:tag}"
      command:
...
asya:
  build:
    - module: e_commerce
      ...
      image: "${image_registry}/ecom:${arg:tag}"
      command:
        local: "docker build -t ${..image} ."
```

**Path resolution**: Two styles coexist for the `path:` field:
- **`${project_root}/...`** -- repo-root-relative via interpolation.
...
`project_root` is defined as `"."` at the top level in the root config.yaml.
...
#   Root:   path: "${project_root}/libs/shared"  →  stays as interpolation
...
asya:
  build:
    ...
    - module: langchain
      image: "ghcr.io/third-party/langchain:v2"
    - module: shared_utils
      path: "${project_root}/libs/shared_utils"   # portable interpolation
      image: "${image_registry}/shared:${arg:tag}"
      command:
        local: "docker build -t ${..image} ."
        remote: "docker build -t ${..image} . && docker push ${..image}"
...
```
.asya/
└── config.yaml               # Full config with constants + asya (images, compile, template)
```
...
asya:
  build: []
    # - module: my_package
    #   path: "${project_root}/src/my-package"
    #   image: "${image_registry}/my-package:${arg:tag}"
    #   command:
    #     local: "docker build -t ${..image} ."
    #     remote: "docker build -t ${..image} . && docker push ${..image}"
...
- **Manifests created on first template**: Output directory
  (`asya.template.output`) is created on first `asya template` invocation, not
...
`asya.build` is a **list** of build entries, each identified by a `module:` field.
...
asya:
  build:
    ...
    - path: "./src/notebooks/models"
      image: "${image_registry}/notebook-models:${arg:tag}"
      command:
        local: "docker build -t ${..image} ."
```
...
are resolved to absolute before merge. Can use `${project_root}/...` for
...
|--------|---------|---------|
| `${path.to.key}` | Absolute path from config root | `${image_registry}` |
...
| `${actor:key}` | Compiler-inferred value (colon resolver) | `${actor:name}` |
...
- **Colon** (`${resolver:key}`) — calls a registered resolver function.
  OmegaConf built-in for `env:`, Asya registers `arg:` and `actor:`.

```python
# Asya registers two custom resolvers at startup:
OmegaConf.register_new_resolver("arg", lambda key: cli_args[key])
OmegaConf.register_new_resolver("actor", lambda key: actor_values[key])
# OmegaConf already provides: ${env:VAR} (oc.env)
```

**Resolution order**: Top-level config references (`${key}`, `${.sibling}`)
are resolved first. Then `${arg:*}`, `${actor:*}`, and `${env:*}` are
resolved at command time.

**Three namespaces**:
- Top-level keys -- static values defined in config. Inherited by child
  configs via walk-up merge. Typically set once in the root config.yaml.
- `${arg:*}` -- runtime values from CLI flags or env vars. No config-level
  definition — they exist only at runtime.
- `${actor:*}` -- values inferred by the compiler at compile time. Not
  user-settable — populated by `asya flow compile` or `asya actor compile`.
  Available keys: `name`, `image`, `handler`, `env`.

**Three override mechanisms** (all generic, zero Asya knowledge):

| Mechanism | Example | Scope |
|-----------|---------|-------|
| CLI flag | `--arg tag=v1`, `--set image_registry=x` | Single command |
| `ASYA_CONFIG_*` env var | `ASYA_CONFIG_ARG_TAG=v1`, `ASYA_CONFIG_IMAGE_REGISTRY=x` | Shell session |
| Config file (top-level) | `image_registry: ghcr.io/org` | Project-wide |

**Env var naming convention**: `ASYA_CONFIG_<NAMESPACE>_<KEY>` where
namespace is `ARG` for runtime args, key is UPPER_SNAKE_CASE of the config
key. Top-level config overrides use `ASYA_CONFIG_<KEY>` directly.
```bash
# These are equivalent:
asya actor build foo --arg tag=v1
ASYA_CONFIG_ARG_TAG=v1 asya actor build foo

# Override a top-level constant from env (useful in CI):
export ASYA_CONFIG_IMAGE_REGISTRY=my-registry.io
asya actor build foo --arg tag=v1
```
...
Error: unresolved interpolation '${arg:tag}'
  in: images[0].image = "${image_registry}/e-commerce:${arg:tag}"
...
asya:
  build:
    - module: e_commerce
      path: "${project_root}/src/e-commerce"
      image: "${image_registry}/e-commerce:${arg:tag}"
    #       ^^^^^^^^^^^^^^^^^^^^^^ → ghcr.io/org  (from top-level)
    #                                              ^^^^^^^^^^^ → v1  (from --arg)
...
**Design**: `router_image` at the top level of the root config.yaml specifies
the default base image for all generated actors. Router code is injected via
ConfigMap (same mechanism as `asya_runtime.py`), so no custom build step is
needed.
...
The compiler generates router manifests referencing `${router_image}`.
...
1. Set `router_image` to a custom image with dependencies pre-installed
2. Or add an `asya.build` entry for it and build it explicitly
...
Error: unresolved interpolation '${registy}'
  ...
  asya.build[0].image = "${registy}/e-commerce:${arg:tag}"
  hint: did you mean '${image_registry}'?
```

**Level 2 — Asya (semantic)**:
- No unknown keys under `asya:` (catch typos like `asya.iamges:`)
- `command.local` / `command.remote` are valid shell commands (basic syntax check)
- `path:` directories exist on disk (when building)
- Image references in manifests resolve to an `asya.build` entry (at compile time)
...
```
Error: unknown key 'iamges' under 'asya:'
...
### 3.8 `asya.compile` and `asya.template` Sections
...
|-------|---------------|-------------|-------------|
| Compile | `asya.compile` | `asya flow compile` | AST → routers + metadata |
| Template | `asya.template` | `asya [flow\|actor] template` | Values → deployment files |
...
`asya.template.output` is the base directory for all generated artifacts.
The templater creates `actors/` and `flows/<name>/` subdirectories under it
as needed. Created on first `asya template` invocation if it doesn't exist.

#### `${actor:*}` resolver keys

| Key | Type | Source |
|-----|------|--------|
| `actor:name` | string | Actor name (derived from handler function name, kebab-cased) |
| `actor:image` | string | OCI image reference (resolved from `asya.build` list by module match) |
...
Output: `<asya.template.output>/flows/<flow>/` or `<asya.template.output>/actors/`.

#### Mode: `helm`

Generates `values.yaml` files for the `asya-actor` Helm chart (or any
...
Output: `<asya.template.output>/flows/<flow>/<actor>/values.yaml`.
...
helm install validate-order asya-actor \
  -f .asya/manifests/flows/order/validate-order/values.yaml
```
...
Output: `<asya.template.output>/flows/<flow>/<actor>/patch.yaml` + generated
`kustomization.yaml` referencing the base.

#### ADR: Integration Direction

**Decision**: Asya integrates INTO helm/kustomize, not reverse.

**Arguments for**:
1. **No plugin maintenance** — Asya generates files that helm/kustomize
...
[template] Mode: manifests (from config asya.template.mode)
...
File paths: ["/proj/src/e-commerce-package/e_commerce/validate.py", ...]
    ↓ match to config.yaml asya.build list (longest module prefix wins)
...
**Output**: deployment files in `<asya.template.output>/`

The templater resolves `template.body` with `${actor:*}` values for each
actor and writes output files. Format depends on `asya.template.mode`:
...
kind: AsyncActor
metadata:
  name: validate-order                              # ${actor:name}
spec:
...
- `asya actor build <actor-name> --local` → reads manifest to find image
  ref → matches image ref to config.yaml `asya.build` entry → runs
...
- Match filesystem paths to `config.yaml` `asya.build` entries
...
asya actor build text-analyzer --set image_registry=my-registry.io
...
# Override a top-level constant from env (useful in CI)
export ASYA_CONFIG_IMAGE_REGISTRY=ci-registry.internal
...
- Top-level keys -- config values (dot syntax, resolved first)
- `${arg:*}` -- CLI args (colon syntax, resolved at command time)
- `${actor:*}` -- compiler-inferred values (colon syntax, resolved at
  compile time only)
...
3. Config file values (child > parent via walk-up merge)
- Top-level keys and `${.sibling}` resolve before `${arg:*}` and `${env:*}`
- `${actor:*}` resolves only during compilation — unavailable in build/deploy
...
3. ~~**Router actor images**~~: Resolved. `router_image` top-level key in
   root config specifies the base image. Router code injected via ConfigMap
...
9. ~~**`asya init` design**~~: Resolved. Static scaffold (like `git init`):
   creates `.asya/config.yaml` with full config (top-level constants +
   `asya.build` + `asya.compile` + `asya.template`). Output directory
   created on first `asya template` invocation, not init. See section 2.4.
...
12. ~~**Opaque build commands completeness**~~: **Resolved (v1: opaque only)**.
    ...
    **v2 extension path** (no schema break): Optional `build.intent:` field
    alongside existing `command.local`/`command.remote` under `asya.build[]`.
    When `intent` exists,
