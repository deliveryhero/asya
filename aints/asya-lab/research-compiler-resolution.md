...
2. **Interpolation resolves AFTER merge**: `${var.*}` references resolve
   ...
   this work.
3. **Colon resolvers (`${arg:*}`, `${compiler:*}`, `${env:*}`) resolve at
   command time**: After merge and after `var` resolution. Missing `arg`
   values are a hard error (no defaults). `${compiler:*}` values are
...
| `${compiler:key}` | Compiler-inferred value (colon resolver) | `${compiler:name}` |
...
- `${compiler:*}` -- values inferred by the compiler at compile time. Not
...
config.yaml. The template uses `${compiler:*}` resolvers for values inferred
...
#### `${compiler:*}` resolver keys

| Key | Type | Source |
|-----|------|--------|
| `compiler:name` | string | Actor name (derived from handler function name, kebab-cased) |
| `compiler:image` | string | OCI image reference (resolved from `images` list by module match) |
| `compiler:handler` | string | Fully qualified Python handler path (`module.function` or `module.Class.method`) |
| `compiler:env` | list | Environment variables extracted from handler code (`os.environ` / `os.getenv` calls) |
...
(section 3.8) with `${compiler:*}` values for each actor:
```yaml
...
kind: AsyncActor
metadata:
  name: validate-order                              # ${compiler:name}
spec:
  image: ghcr.io/org/e-commerce:${arg:tag}          # ${compiler:image}
  handler: e_commerce.validate.validate_order       # ${compiler:handler}
...
- `${var.*}` -- config tree (dot syntax, resolved first)
- `${arg:*}` -- CLI args (colon syntax, resolved at command time)
- `${compiler:*}` -- compiler-inferred values (colon syntax, resolved at
  compile time only)
...
- `${compiler:*}` resolves only during compilation — unavailable in build/deploy
...
15. **Compiler template: `${compiler:env}` format**: Environment variables
...
16. **Infrastructure defaults ownership**: Overlays handle infrastructure
    ...
    (`${compiler:*}`) and leave all static defaults to overlays?
