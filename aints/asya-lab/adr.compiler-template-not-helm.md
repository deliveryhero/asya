...
The compiler uses a configurable YAML template with `${actor:*}` OmegaConf
resolvers to generate output (manifests, helm values, kustomize patches). This
is printf-level substitution, not a template engine.
...
| Arbitrary Go template expressions | Fixed set of ~5 compiler values |

The compiler has exactly these values to place:

- `${actor:name}` — actor name (derived from handler, kebab-cased)
- `${actor:image}` — resolved OCI image reference
- `${actor:handler}` — fully qualified Python handler path
- `${actor:env}` — environment variables extracted from handler code
...
the same mechanism used everywhere else in config.yaml.
