...
| W7 | **Smart entrypoint detection** — first user actor is entrypoint, no empty start router. Multiple exitpoints possible (actors before each `return`). | Eliminates no-op start/end routers (aint `20c9`). |
...
| G3 | **`flow_role` vocabulary: `entry`, `exit`, `entryexit`, `router`, `actor`** — used in both graph.json and `asya.sh/flow-role` K8s label. | Unified vocabulary across graph, manifests, and K8s labels. Needs XRD-level validation. |
